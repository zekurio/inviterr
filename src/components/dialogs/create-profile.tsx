...
export function CreateProfileDialog({ isOpen, onOpenChange }: CreateProfileDialogProps) {
    ...
    const createMutation = api.profiles.create.useMutation({
        ...
        onError: (error) => {
            const message = error.data?.zodError?.fieldErrors.jellyfinTemplateUserId?.[0]
                ?? error.message
        ...
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Configure a new profile for generating invites. Optionally link to a Jellyfin user as a template.
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Select 
                                        ...
                                        disabled={jellyfinUsersQuery.isLoading || jellyfinUsersQuery.isError}
                                    >
                                        <FormControl>
                                        ...
                                        <SelectContent>
                                            <SelectGroup>
                                                {jellyfinUsersQuery.isError && (
                                                    <div className="p-4 text-destructive text-sm">Error loading users.</div>
                                                )}
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit" disabled={createMutation.isPending || jellyfinUsersQuery.isLoading}>
