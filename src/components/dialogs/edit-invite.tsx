...
export function EditInviteDialog({ invite, isOpen, onOpenChange }: EditInviteDialogProps) {
        ...
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Update details for invite code: <span className="font-mono font-semibold">{invite.code}</span>
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                                     ...
                                     <Controller
                                        ...
                                        render={({ field: controllerField }) => (
                                            <DatePicker 
                                                ...
                                                setDate={(date) => controllerField.onChange(date ?? null)}
                                            />
                                        )}
                                     />
                                     <FormDescription>
                                         Current: {invite.expiresAt ? new Date(invite.expiresAt).toLocaleDateString() : "Never"}
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )}
                         ...
                         <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormDescription>
                                        Leave blank for unlimited uses. Current: {invite.maxUses ?? "Unlimited"}
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )}
